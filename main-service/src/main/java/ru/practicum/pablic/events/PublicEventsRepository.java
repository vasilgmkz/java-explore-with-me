...
public interface PublicEventsRepository extends JpaRepository<EventDto, Long>, QuerydslPredicateExecutor<EventDto> {
    ...
    @Query(value = "select * from events where (lower(event_annotation) like LOWER(CONCAT('%',:text ,'%')) \n" +
            ...
            "and case when :paid = true then event_paid = true  when :paid = false then event_paid = false else event_paid = event_paid end\n" +
            "and event_state = 'PUBLISHED'\n" +
            "and distance(:lat, :lon, event_location_lat, event_location_lon) <= :radius\n" +
            "and event_event_date between :rangeStart and :rangeEnd\n" +
            "OFFSET :from LIMIT :size ", nativeQuery = true)
    List<EventDto> getEventsWithLocation(@Param("text") String text,
                                         @Param("categories") List<Integer> categories,
                                         @Param("paid") Boolean paid,
                                         @Param("rangeStart") LocalDateTime rangeStart,
                                         @Param("rangeEnd") LocalDateTime rangeEnd,
                                         @Param("from") Long from,
                                         @Param("size") Long size, @Param("lat") Float lat, @Param("lon") Float lon, @Param("radius") Float radius);
}
